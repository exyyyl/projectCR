...
interface Props {
  ...
  onApply: (crosshair: Crosshair) => void
...
export function CrosshairCard({ crosshair, onDelete, onApply, onCopyCode }: Props) {
    ...
    <div className="group bg-amoled-surface border border-amoled-border hover:border-amoled-border-strong rounded-2xl overflow-hidden transition-all duration-150">
      ...
      <div className="flex items-center justify-center bg-black py-5 relative">
        ...
        <div className="absolute top-1.5 right-1.5 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <TipBtn label="Применить в игре" onClick={() => onApply(crosshair)}>
            <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor">
              <path d="M2 1.5l7 3.5-7 3.5V1.5z" />
            </svg>
          </TipBtn>

          <TipBtn label="Удалить" onClick={() => onDelete(crosshair.id)} danger>
